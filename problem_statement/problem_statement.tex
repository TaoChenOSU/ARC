...
prohibitive for the average consumer. The purpose of the Autonomous RC (ARC)
project is to determine if it is possible to build an autonomous RC vehicle
using commodity hardware.  This will be accomplished using commodity hardware,
including cameras, computers, and the RC vehicle used as the base, with minimal
fabrication.\newline
\newline\newline
...
\section{Motivation}
Millions, if not billions of dollars have been allocated for research of
autonomous vehicles. A lot of the hardware is very specialized, and often very
expensive. Our challenge is to see if it is possible to replicate a small subset
of the expensive system's abilities, at a small-scale, using commodity hardware
that costs hundreds of dollars instead of hundreds of thousands. Being able to
implement even a small subset of the features at a small scale, could
potentially mean a feature could also be scaled for larger implementations. A
small-scale autonomous vehicle, built with commodity hardware, would provide an
inexpensive way to test autonomous features and the viability of new research in
the autonomous vehicle (AV) field, even for a hobbyist user.
